classdef Planewave < acq.mconfig
	% Configuration using plane wave sources.
	%
	% All receivers are equally distributed on a circle, and the plane wave sources cover uniformly 
	% [0,2pi]. The sources are identified by their directions.
	
	properties(SetAccess = protected)
		radius_src = 1 % radius of the virtual measurement circle for sources
	...
	methods
		function obj = Planewave(Z, Rr, Nr, Ns, viewmode, grouped)
			...
			% -grouped: if true the sources/receivers of different arcs become
			% independent
