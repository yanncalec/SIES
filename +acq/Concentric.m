classdef Concentric < acq.mconfig
	...
	% circle and the circle of sources and receivers are concentric. Sources are eventually divided into
	% groups (in form of arcs), so that there is no communication between different groups, 
	% i.e. the receivers of one group cannot listen to sources of another
	% group. The source(s) can be a single dirac of multiple diracs satisfying the neutrality condition.
	...
	properties(SetAccess = protected)
		...
		nbDirac % Neutrality condition: f(x) = sum_{j=1^nbDirac} a_j dirac(x-x_{s,j}), with sum_j a_j = 0
		neutRad % the positions of dirac x_{s,j} are distributed on a tangent segment at x_s with the 
		        % length being proportional to neutRad, see the method neutSrc()
	...
	methods
		function obj = Concentric(Z, Rs, Ns, Rr, Nr, viewmode, grouped, neutCoeff, neutRad)
			...
			% -grouped: if true each arc will be treated as an independant group (limited view),
			% if false the whole set of receivers will be visible for all sources (full view but sparse array).
