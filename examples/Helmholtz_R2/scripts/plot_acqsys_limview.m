%% Plot the acquisition systems of limited view and the SVD of the operators
...
N0 = 18; % Number of sources (and receivers)
cfg = acq.Planewave(91, mcenter, mradius, N0, [5, 0.3*pi, 2*pi]);
...
saveas(fig, ['../figures/Acqsys_limview.eps'], 'psc2');
...
for ord = 20:10:40
    Op.Ar = PDE.Helmholtz_R2.make_matrix_Src(P.wavenb_bg, cfg.all_rcv{1}, cfg.center, ord);
    Op.As = PDE.Helmholtz_R2.make_matrix_Src(P.wavenb_bg, cfg.all_src, cfg.center, ord);
    [uu, ss, vv] = svd(Op.As);
...
saveas(fig1, '../figures/SVD_limview.eps','psc2');
