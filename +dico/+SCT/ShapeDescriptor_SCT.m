function [S, G] = ShapeDescriptor_SCT(W, Nv, bdwidth)
...
if nd==2
    [S, G] = ShapeDescriptor_SCT_onefreq(W, Nv, bdmask);
...
elseif nd==3
    ...
    for n=1:Nf
        [S(:,:,n), G(:,:,n)] = ShapeDescriptor_SCT_onefreq(W(:,:,n), Nv, bdmask);
    end
else
    error('The dimension of the input W must be 2 or 3!');
end

function [S, G] = ShapeDescriptor_SCT_onefreq(W, Nv, bdmask)
% Invariant shape descriptor based on Scattering coefficients
% Inputs:
% W: SCT (square) matrix
% Nv: sampling points for the square [0,2pi]^2, Nv=256 by default
...
G = dico.SCT.farfieldpattern(W,Nv);
