function [F, F_bg, Sx, Sy, mask] = calculate_field(obj, n, s, z0, width, N, fpsi_bg, fpsi, fphi)
...
% SX, SY: coordinates in x and y-axis of the rectangular region
    ...
    % Multiplication by h(t) since the source is separable   
    Hs = reshape(Gx*dipole(1)+Gy*dipole(2), 1, []) * obj.waveform(n);
    
    % Multiplication by h(t) since the background field is separable       
    fpsi_bg = fpsi_bg(:, s) * obj.waveform(n);
    fpsi = fpsi{n}(:, s);
    fphi = squeeze(fphi{n}(:, s, :));
